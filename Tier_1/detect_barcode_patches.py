...
#@ Boolean(label="Use toto",value=4) use_toto
...
for root, dirs, files in walk(input_folder):
    for file in files:
    	if file.endswith(".tiff") or file.endswith(".tif"):
			...
			#roiMng = RoiManager.getInstance()
			nslices = imp.getNSlices()
			#nchannels = imp.getNChannels()-1
			...
			#pixel_data = [[[0]*width*height]*nslices]*(nframes+2)
			...
			for frame in range(1,nframes+1):
				for c in range(1,nchannels+1):
					...
					#histo = histogram(pixels,1000)
					#cumip = reduce(lambda c, x: c + [c[-1] + x], histo, [0])[1:]
					#median_pix = next(x[0] for x in enumerate(cumip) if x[1] > 0.99 * len(pixels))
					#p99[(frame,c)] = (median_pix/1000.)*max_max
					p99[(frame,c)] = max_max
					#pmode[(frame,c)] = max_mode
					pmode[(frame,c)] = 0
					#max_glob = (max_max-max_mode)/((median_pix/1000.)*max_max-max_mode)
					max_glob = 1.0
					if max_glob > max_global_intensity:
						max_global_intensity = max_glob

			roimanager = RoiManager(True)
			#roimanager.runCommand("Select None")
			...
			#for roi_idx in [0]:
				...
				for frame in xrange(nbr_rounds):
					...
					#frameImp = Duplicator().run(imp, 1, nchannels, 1, nslices, frame+1, frame+1)
					#IJ.save(frameImp, '%s_round_%i.tif' % (file_pattern,frame))
					#roimanager.runCommand(frameImp, "Combine")
					
					#sliceImp.show()
					#sys.exit()
					#stack = sliceImp.getStack()
					...
					for i in xrange(nslices):
						...
						[0]*3*nframes)} for _ in xrange(nslices)]
								...
								#if pixel_data[roi_idx][p][i][nframes+frame] < pix_intensity:
									#pixel_data[roi_idx][p][i][nframes+frame] = pix_intensity
									#pixel_data[roi_idx][p][i][frame] = c
			...
			pix_int = {}
			#patches = dict((barc,[]) for barc in barcodes)
			patches = {}
			...
			max_area_pix = 5
			#base_colors = [colorToRGB[col] for col in colors]
