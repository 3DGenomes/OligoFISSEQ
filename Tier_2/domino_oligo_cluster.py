...
class ChromosomesTerritoryRestraint(IMP.Restraint):
    ...
    def __init__(self, m, particles, kforce, chromosome_distance):
        ...
        self.partj = []
        for chr_nbr in xrange(len(particles)):
            parts = []
            for i in xrange(len(particles[chr_nbr])):
                parts.append(IMP.core.XYZ(m,particles[chr_nbr][i]))
            self.partj.append(parts)

    def do_add_score_and_derivatives(self, sa):
        score = 0
        for chr_nbr in xrange(len(self.partj)-1):
    ...
    def do_get_inputs(self):
        parts = []
        for chr_nbr in xrange(len(self.partj)):
...
def get_centers(chrom_barcodes, nbr_chrs, chr_territory, min_cluster_separation,
                max_locis_per_barcode = 6, min_loci_per_chr=2, exclude_centers=[]):
    
    pix_territory = int(chr_territory/pixelWidth)
    pix_min_cluster_separation = int(min_cluster_separation/pixelWidth)
    
    points_barcodes = [[patches[barcode][b_id] for b_id in best_patches_ids[barcode][:max_locis_per_barcode]]
                           for barcode in chrom_barcodes]
    #points = [[np.array([int(pb_i['center_pixel_x']/pixelWidth),
    #                     int(pb_i['center_pixel_y']/pixelWidth),
    #                     int(pb_i['center_pixel_z']/pixelDepth)])
    #                     for pb_i in points_barcode]
    #                     for points_barcode in points_barcodes]
    points = [item for sublist in points_barcodes for item in sublist]
    for cent in exclude_centers:
        points = [pb_i for pb_i in points 
                    if np.linalg.norm(np.array([pb_i['center_pixel_x']/pixelWidth,
                                                pb_i['center_pixel_y']/pixelWidth])-np.array([cent[0]/pixelWidth,
                                                                                             cent[1]/pixelWidth])) > pix_territory]
    if len(points) == 0:
        return None
    avg_z = np.mean([np.array(pb_i['center_pixel_z']) for pb_i in points])
    positions_xy = [(int(pb_i['center_pixel_x']/pixelWidth),
                  int(pb_i['center_pixel_y']/pixelWidth)) for pb_i in points]
    
    max_area = 0
    optim_centers = []
    for centers in combinations(positions_xy,nbr_chrs):
        points_inside_1 = [pb_i for pb_i in points
                        if np.linalg.norm(np.array([pb_i['center_pixel_x']/pixelWidth,
                                                    pb_i['center_pixel_y']/pixelWidth])-np.array(centers[0])) < pix_territory]
        points_np_1 = np.array([[int(pb_i['center_pixel_x']/pixelWidth),
                                         int(pb_i['center_pixel_y']/pixelWidth)]
                                      for pb_i in points_inside_1])
        cen_len_1 = len(set([pb_i['barcode'] for pb_i in points_inside_1]))
        area_1 = sum([pb_i['nbr_pixels'] for pb_i in points_inside_1])
            
        if nbr_chrs > 1:
            points_inside_2 = [pb_i for pb_i in points 
                               if np.linalg.norm(np.array([pb_i['center_pixel_x']/pixelWidth,
                                        pb_i['center_pixel_y']/pixelWidth])-np.array(centers[1])) < pix_territory]

            points_np_2 = np.array([[int(pb_i['center_pixel_x']/pixelWidth),
                                     int(pb_i['center_pixel_y']/pixelWidth)]
                                  for pb_i in points_inside_2])
            cen_len_2 = len(set([pb_i['barcode'] for pb_i in points_inside_2]))
            area_2 = sum([pb_i['nbr_pixels'] for pb_i in points_inside_2])

            centers_np = (np.mean(points_np_1, axis=0),np.mean(points_np_2, axis=0))
            
            if np.linalg.norm(centers_np[0]-centers_np[1]) < pix_min_cluster_separation:
                continue
        
            if max_area < cen_len_1 + cen_len_2 and cen_len_1 > min_loci_per_chr and cen_len_2 > min_loci_per_chr:
                max_area = cen_len_1 + cen_len_2
                optim_centers = centers_np 
            #if max_area < area_1 + area_2 and cen_len_1 > 2 and cen_len_2 > 2:
            #    max_area = area_1 + area_2
            #    optim_centers = centers_np
        else:
            centers_np = np.mean(points_np_1, axis=0)
            if max_area < cen_len_1 and cen_len_1 > min_loci_per_chr:
                max_area = cen_len_1
                optim_centers = centers_np
    
    if nbr_chrs > 1:
        if len(optim_centers) == 0:
            optim_centers = [np.mean([np.array([pb_i['center_pixel_x']/pixelWidth,
                                                pb_i['center_pixel_y']/pixelWidth]) for pb_i in points], axis=0)]
        optim_centers = [np.array([oc[0]*pixelWidth,oc[1]*pixelWidth,avg_z]) for oc in optim_centers]
    else:
        if len(optim_centers) == 0:
            optim_centers = np.mean([np.array([pb_i['center_pixel_x']/pixelWidth,
                                                pb_i['center_pixel_y']/pixelWidth]) for pb_i in points], axis=0)
        optim_centers = [np.array([optim_centers[0]*pixelWidth,optim_centers[1]*pixelWidth,avg_z])]
...
if __name__ == "__main__":
    ...
    chr_join_max_dist_46plex = 2
    chr_join_max_dist_23plex = 2.5
    chr_territory = 4.5
    pixelWidth = 0.267
    pixelDepth = 0.3
    ImageWidth = 512
    ImageHeight = 512
    max_points_domino = 4
    barcode_step = 150 if len(chrs) > 1 else 20
    ...
    for roi_idx in selected_roi:
        ...
        for chrom_idx, chrom in enumerate(chrs):
            ...
            if len(chrom_barcodes) > 6:
                min_loci_per_chr = 6
            #min_loci_per_chr = int(len(chrom_barcodes)/4)
