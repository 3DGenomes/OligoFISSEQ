...
macro "Create hyperstack from rounds"  {
 ...
 dirPath = getArgument();
 print(dirPath);
 FolderTemplatePath=dirPath+File.separator+"round_";
 TotoPath=dirPath+"toto";
 outputPath = dirPath+File.separator+"merged"+File.separator;
 seg_channel=5;
 toto_seg_channel=5;
 y_drift_c2=-1;
...
function create_hyperstack_from_rounds(FolderTemplatePath, TotoPath, outputPath, seg_channel, toto_seg_channel, y_drift_c2) {
  ...
  for (i = 0; i < list_round1.length; i++) {
    ...
    run("Watershed", "slice");
    //run("Find Edges", "slice");
    run("Analyze Particles...", "size=50.00-Infinity add slice");
